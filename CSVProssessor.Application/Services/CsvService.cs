...
public class CsvService : ICsvService
{
    ...
    public async Task<ImportCsvResponseDto> ImportCsvAsync(IFormFile file)
    {
        ...
        return new ImportCsvResponseDto
        {
            ...
            Message =
                $"File CSV đã được tải lên thành công với tên: {uniqueFileName}. Background service sẽ xử lý trong thời gian sớm nhất."
        };
    }
    ...
    public async Task<(string fileName, byte[] fileBytes)> ParseMultipartFormDataAsync(string contentType, Stream body)
    {
        ...
        if (string.IsNullOrEmpty(boundary)) boundary = ExtractBoundaryFromBody(memoryStream);
    ...
    public async Task<Stream> ExportAllCsvFilesAsync()
    {
        ...
        {
            foreach (var fileName in uniqueFileNames) await AddFileToZipArchiveAsync(archive, fileName);
        }
    ...
    private string? ExtractBoundaryFromBody(MemoryStream stream)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(firstLine) &&
                firstLine.StartsWith("--")) return firstLine.Substring(2); // Remove "--"
        }
        catch (Exception)
        {
            return null;
    ...
    private async Task<(string? fileName, byte[]? fileBytes)> ParseMultipartFormData(MemoryStream stream,
        string boundary)
    {
        try
        {
            ...
            foreach (var part in parts)
                if (part.Contains("Content-Disposition") && part.Contains("filename="))
                {
                    ...
                    if (!fileNameMatch.Success)
                        fileNameMatch = Regex.Match(part, @"filename=([^\r\n;]+)");

                    var fileName = fileNameMatch.Success ? fileNameMatch.Groups[1].Value.Trim() : "uploaded_file.csv";
    ...
    private async Task<List<CsvRecord>> ParseCsvAsync(Guid jobId, string fileName, Stream fileStream)
    {
        ...
        while ((line = await contentReader.ReadLineAsync()) != null) lines.Add(line);
        ...
        var isHeaderless = IsDataLine(firstLineValues);
        ...
        for (var i = dataStartIndex; i < lines.Count; i++)
        {
            ...
            for (var j = 0; j < headers.Length && j < values.Length; j++) jsonDict[headers[j]] = values[j];
    ...
    private bool IsDataLine(string[] values)
    {
        ...
        foreach (var value in values)
        {
            ...
            if (Regex.IsMatch(value, @"^[A-Z0-9]{8,}$"))
                dataIndicators++;
