...
public class DatabaseChangeNotificationFunction
{
    ...
    public async Task RunAsync([TimerTrigger("0 */1 * * * *")] TimerInfo myTimer)
    {
        ...
        catch (Exception ex)
        {
            ...
            _loggerService.Error($"[FuncApp1] Error in DatabaseChangeNotificationFunction: {ex.Message}");
        }
    }
        ...
        try
        {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                try
                {
                    ...
                    _logger.LogInformation("[FuncApp1] Received change notification: {message}", json);
                    _loggerService.Info($"[FuncApp1] Received change notification: {json}");

                    // Send email from FuncApp1
                    var emailRequest = new EmailRequestDto
                    {
                        To = "phuctg1@fpt.com"
                    };

                    await _emailService.SendDatabaseChangesWithSource(emailRequest, "FuncApp1 - CSV Import Processor");
                    _loggerService.Success("[FuncApp1] Email sent successfully for database change");
                ...
                {
                    _logger.LogError(ex, "[FuncApp1] Error processing message");
                    _loggerService.Error($"[FuncApp1] Error processing message: {ex.Message}");
            ...
            await channel.BasicConsumeAsync(
                queueName,
                false,
                $"FuncApp1-{Guid.NewGuid():N}",
                consumer);

            _logger.LogInformation("[FuncApp1] Started listening to csv-changes-topic");
            ...
            if (messageReceived)
            {
                _logger.LogInformation("[FuncApp1] Message processed successfully");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[FuncApp1] Error subscribing to topic");
            _loggerService.Error($"[FuncApp1] Error subscribing to topic: {ex.Message}");
        }
    }
}
